...
export const getCurrentCity = () => {
  // 3 判断 localStorage 中是否有定位城市  JSON.parse 是将 字符串 转化为 对象
  const localCity = JSON.parse(localStorage.getItem('hkzf_city'))
  if (!localCity) {
    // 4 如果没有，就使用首页中获取定位城市的代码来获取，并且存储到本地存储中，然后返回该城市数据      Promise 成功会执行 resolve 失败 会执行 reject
    return new Promise((resolve, reject) => {
      ...
      curCity.get(async res => {
        try {
          ...
          // 存储到本地存储中 JSON.stringify 是将 对象转化为字符串
